    ...
    def build_children(self, model):
        ...
        for space in range(len(empty_spaces)):
            ...
            for car in cars:
                ...
                archive[new_model] = new_move
                self.states.append(new_model)


    def is_solution(self, new_model):
        """
        The game is solved if the winning move is included in the states.
        """
        if len(new_model.board[0]) == 6:
            victory_coor = [5, 2]
        elif len(new_model.board[0]) == 9:
            victory_coor = [8, 4]
        elif len(new_model.board[0]) == 12:
            victory_coor = [11, 5]

        #print(board[victory_coor[1]][victory_coor[0]])

        if new_model.board[victory_coor[1]][victory_coor[0]] == 'X':
            return True
        else:
            return False
